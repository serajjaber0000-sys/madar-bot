<template>
  <div class="bk-page">
    <nav class="bk-nav">
      ...
      <button class="bk-nav-bell" @click="showNotifications = !showNotifications" :class="{ active: showNotifications }">
    ...
    <Transition name="bk-notif">
      <div v-if="showNotifications" class="bk-notif-panel">
        <div class="bk-notif-header">
          ...
          <button v-if="patientNotifications.length > 0" class="bk-notif-clear" @click="markAllRead">تحديد الكل كمقروء</button>
        </div>
...
import { collection, query, where, onSnapshot, orderBy, updateDoc, doc as fsDoc } from 'firebase/firestore'
...
<style scoped>
.bk-page{font-family:'Segoe UI',Tahoma,Arial,sans-serif;direction:rtl;color:#1e293b;background:#f0f2f5;min-height:100vh;min-height:100dvh;padding-bottom:80px;-webkit-font-smoothing:antialiased;-webkit-overflow-scrolling:touch}
...
.bk-nav-title{flex:1;text-align:center;font:800 .95rem 'Segoe UI',sans-serif;color:#fff}
.bk-nav-bell{position:relative;width:40px;height:40px;border-radius:10px;display:grid;place-items:center;color:#fff;background:none;border:none;cursor:pointer;transition:background .15s}
.bk-nav-bell:active,.bk-nav-bell.active{background:rgba(255,255,255,.2)}
.bk-bell-badge{position:absolute;top:4px;left:4px;min-width:18px;height:18px;padding:0 5px;border-radius:9px;background:#ef4444;color:#fff;font:700 .65rem 'Segoe UI',sans-serif;display:grid;place-items:center;line-height:1}
...
.bk-notif-title{font:800 .88rem 'Segoe UI',sans-serif;color:#1e293b}
.bk-notif-clear{font:600 .72rem 'Segoe UI',sans-serif;color:#0d9488;background:none;border:none;cursor:pointer}
...
.bk-notif-dot{width:8px;height:8px;border-radius:50%;flex-shrink:0;margin-top:5px}
...
.bk-empty-link{display:inline-block;padding:12px 28px;border-radius:12px;background:#0d9488;color:#fff;text-decoration:none;font:700 .88rem 'Segoe UI',sans-serif}
...
.bk-card-doctor{font:800 .92rem 'Segoe UI',sans-serif;color:#1e293b;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
